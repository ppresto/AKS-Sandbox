...
if [[ $(terraform output -state=./terraform.tfstate K8s_namespace) ]]; then
...
initializeVault () {
    ...
    if [[ $(echo $vaultInitStatus | awk '{ print $NF }' | grep false) ]]; then
        ...
        echo
        kubectl exec --kubeconfig ${config} --namespace ${ns} ${init_inst} -- vault operator init -key-shares=1 -key-threshold=1 -format=json > tmp/cluster-keys.json
        sleep 5
